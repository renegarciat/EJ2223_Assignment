...
- ~~The shaft diameter (D_ir) is not being pushed to COMSOL~~ — fixed: `IPMRotorSizer.RotorID_mm` was already being calculated, just never pushed. `ComsolPrebuiltInterface.pushGeometry` now accepts `ShaftDiameter_mm` and sets it on the model's existing `d_s` Global Parameter (already wired to `pi1`'s `shaft_diam` input in the template, just left at a stale 10mm literal); `main.m` passes `motorGeometry.RotorInnerRadius_m * 2e3`.