...
- `pushGeometry(Np=, Ns=, Airgap_mm=, StatorYokeHeight_mm=, RotorDiameter_mm=, L_mm=, MaxSpeed_rpm=)` — Sets Global Parameters and re-pins the Part Instance inputs known to need a symbolic reference (e.g. `backiron_th → h_sy`). `MaxSpeed_rpm` drives `w_rot`, the rotation speed the coupled structural sweep evaluates the rotor-bridge stress at -- re-pinned on every push for the same "can't silently drift" reason as `backiron_th`.
