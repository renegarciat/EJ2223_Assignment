...
%     Bt_T                Tooth flux density target     (default 1.415 T)
%     Bc_T                Back-iron flux density target (default 1.00 T)
%     LinearCurrentDensity_Am   RMS linear current density Ks,1 [A/m] (default 90e3)
...
%     FluxWeakeningRatio  MaxSpeed_rpm / CornerSpeed_rpm
%
%   Example
%   -------
%     spec = MotorSpec(200, 2900, 13500, 650, 8, 60, 1, ...
...
%   See also: IPMRotorSizer, EssonSizer, MotorGeometry
    ...
    properties
        ...
        % Bt_T and Bc_T are back-solved from the paper's own worked
        % example (eqs. 85, 93: b_ts=5.89mm and h_sy=20.0mm at the D=160mm,
        % Bg1o=0.965T reference point) rather than generic engineering
        % defaults -- see MATLAB_Code/tests/test_StatorWindingSizing.m.
        Bg1_T               (1,1) double {mustBeInRange(Bg1_T,    0.5, 1.5)} = 0.95
        Bt_T                (1,1) double {mustBeInRange(Bt_T,     1.0, 2.5)} = 1.415
        Bc_T                (1,1) double {mustBeInRange(Bc_T,     0.5, 2.0)} = 1.00

        % Electrical loading
        % LinearCurrentDensity_Am (A1) is Ks,1, the RMS linear current
        % density (Lecture 6, "The Esson's rule"): sigma_m = A1*Bg1/sqrt(2)
        % in EssonsSizer.compute_ is only dimensionally consistent with the
        % lecture's sigma_m = Bg1*Ks,1/2 if A1 is RMS (Ks,1 there is built
        % from a peak Is, so Ks,1 = sqrt(2)*A1_rms). See EssonsSizer.m.
    ...
    methods
        ...
        function obj = MotorSpec(torque_Nm, cornerSpeed_rpm, maxSpeed_rpm, ...
            ...
            arguments
                ...
                options.Bt_T                 (1,1) double = 1.415
                options.Bc_T                 (1,1) double = 1.00
            ...
            if obj.AspectRatio < 1.0 || obj.AspectRatio > 2.0
                warning('MotorSpec:invalidAspectRatio', 'The aspect ratio (%.2f) is outside the typical range of 1.0-2.0.', obj.AspectRatio);
            end
        end
    ...
    methods (Access = public)
        ...
        function summary(obj)
            ...
            fprintf('\n');
            fprintf('  %-30s %g degC\n', 'PM operating temp:',  obj.PMTemp_C);
            fprintf('  %-30s %.2f %%/degC\n', 'Br temp. coefficient:', obj.kBr_pctPerC);
