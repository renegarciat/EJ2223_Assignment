function define_materials(model, comp_tag, phys_tag, draw_only_sector, mesh_size, ...
    ...
    %% ---- 2. All materials ----------------------------------------------
    function mat = def_mat(tag, name, mu, sigma, epsilon, selection)
        ...
        mat.materialmodel('def').set('relpermittivity', epsilon);    
        mat.selection.named(selection);
    end

    % Shaft
    def_mat('mat_shaft', 'Shaft', mu_r_shaft, sigma_shaft, epsilon_r_shaft, 'sel_shaft');
    fprintf('  Shaft relative permeability     : %.2f\n', mu_r_shaft);
    fprintf('  Shaft conductivity              : %.2f S/m\n', sigma_shaft);
    fprintf('  Shaft relative permittivity     : %.2f\n', epsilon_r_shaft);

    % Iron
    def_mat('mat_iron', 'Iron', mu_r_iron, sigma_iron, epsilon_r_iron, 'sel_iron');
    fprintf('  Iron relative permeability      : %.2f\n', mu_r_iron);
    fprintf('  Iron conductivity               : %.2f S/m\n', sigma_iron);
    fprintf('  Iron relative permittivity      : %.2f\n', epsilon_r_iron);

    % Copper
    def_mat('mat_copper', 'Copper', mu_r_copper, sigma_copper, epsilon_r_copper, 'sel_coils');
    fprintf('  Copper relative permeability    : %.2f\n', mu_r_copper);
    fprintf('  Copper conductivity             : %.2f S/m\n', sigma_copper);
    fprintf('  Copper relative permittivity    : %.2f\n', epsilon_r_copper);

    % Air
    def_mat('mat_air', 'Air', mu_r_air, sigma_air, epsilon_r_air, 'sel_airgap_pockets');
    fprintf('  Air relative permeability       : %.2f\n', mu_r_air);
    fprintf('  Air conductivity                : %.2f S/m\n', sigma_air);
    fprintf('  Air relative permittivity       : %.2f\n', epsilon_r_air);

    % Magnets
    mat_mag = def_mat('mat_magnets', 'Magnets', mu_r_magnets, sigma_magnets, epsilon_r_magnets, 'sel_rotor_magnets');
    ...
    fprintf('  Magnets relative permittivity   : %.2f\n', epsilon_r_magnets);
    ...
    if draw_only_sector
        perR = phys.feature.create('periodic_conditions_rotor', 'PeriodicCondition', 1);
        perR.label('Rotor periodic conditions');
        perR.selection.named('bnd_rotor_sides');
        perR.set('PeriodicType', 'AntiPeriodicity');

        perS = phys.feature.create('periodic_conditions_stator', 'PeriodicCondition', 1);
        perS.label('Stator periodic conditions');
        perS.selection.named('bnd_stator_sides');
        perS.set('PeriodicType', 'AntiPeriodicity');
    end

    amp = phys.feature.create('ampere_law_condition', 'AmperesLawSolid', 2);
    amp.label("Ampère's law");
    amp.selection.named('sel_all_without_copper');
