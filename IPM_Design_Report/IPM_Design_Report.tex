...
\end{equation}
where $\rho_{pm}$ is the magnet density, $w_m=4.97\,\text{mm}$ (derived, eq.~\ref{eq:wm} -- Table \ref{tab:comsol_param_set}), $t_m=3\,\text{mm}$, and $l_e$ is the active stack length.
...
For traceability, Table~\ref{tab:comsol_param_set} summarizes the \textbf{current} geometry and operating-point parameters, and the \texttt{MotorSpec}/\texttt{IPMRotorSizer} property each one maps to. \texttt{IPMRotorSizer.solve()} now converges end-to-end for this design point (\texttt{main.m}: \texttt{AlphaM=0.80}, \texttt{Whr\_fraction=0.1}, \texttt{Hm\_mm=3}), so the table reports its actual solved output rather than the earlier placeholders -- including the magnet/bridge geometry (last four rows), which Section \ref{sec:vshape_definition} previously hand-set. The stack length here ($l_e=46.6$\,mm, eq.~\ref{eq:stack_length}) is Ricca's own electromagnetically-driven result, distinct from Formula 1/2's Esson-rule estimates (Section \ref{sec:ricca_methodology} reconciliation discussion) -- the two sizing passes have not yet been cross-checked against each other.
